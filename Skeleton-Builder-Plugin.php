...
class Skeleton_Builder_Plugin extends \WDE\PluginLibrary\Plugin {
	...
	public function admin_page_js(){
		...
		$nested_sortable	= 1 ? '/js/jquery.mjs.nestedSortable.js'	: '/js/jquery.mjs.nestedSortable.min.js';
		$skeleton			= 1 ? '/js/skeleton.js'						: '/js/skeleton.min.js';
		$spin				= 1 ? '/js/spin.js'							: '/js/spin.min.js';
		$jquery_spin		= 1 ? '/js/jquery.spin.js'					: '/js/jquery.spin.min.js';
		$autogrow_textarea	= 1 ? '/js/jquery.autogrow-textarea.js'		: '/js/jquery.autogrow-textarea.min.js';
	...
	public function admin_page(){
		...
		$skeleton = ( isset( $_POST['skeleton'] ) && $_POST['skeleton'] != '' ) ? trim( $_POST['skeleton'] ) : '';
		$skeleton_menu = ( isset( $_POST['skeleton_menu'] ) && $_POST['skeleton_menu'] != '' ) ? trim( $_POST['skeleton_menu'] ) : '';
		$skeleton_post_type = ( isset( $_POST['skeleton_post_type'] ) && $_POST['skeleton_post_type'] != '' ) ? $_POST['skeleton_post_type'] : 'page';
	...
	function process( $skeleton, $skeleton_menu = '', &$menu_id = false, $skeleton_post_type = 'page' ){
		...
		foreach( $line_items as $index => $p ){
			...
			$p = preg_replace( $depth_identifier, '', $p );
			@list( $slug, $title ) = explode(':', $p );
	...
	public function ajax_process(){
		...
		$skeleton_menu	= filter_has_var( INPUT_POST, 'skeleton_menu') ? sanitize_text_field( $_POST['skeleton_menu'] ) : '';
		$skeleton		= null;

		if( filter_has_var( INPUT_POST, 'skeleton_builder_action' ) && ! wp_verify_nonce( $_POST['skeleton_builder_action'], 'build-skeleton' ) )
			$this->errors['nonce'] = 'empty or invalid nonce value';

		if( filter_has_var( INPUT_POST, 'skeleton_post_type') ){

			$pt = get_post_type_object ( trim( $_POST['skeleton_post_type'] ) );
		...
		if( filter_has_var( INPUT_POST, 'skeleton' ) ){
			...
			$json = str_replace("\\", "", $_POST['skeleton'] );
